...
class ClassificationDataset(torchvision.datasets.ImageFolder):
    ...
    def __init__(self, root, augment, imgsz, cache=False):
        ...
        class_map_file = next(
            (p for p in (root.parent / "classes.txt", root.parent / "classes.names") if p.exists()),
            None,
        )
        if class_map_file:
            names = [x.strip() for x in class_map_file.read_text().splitlines() if x.strip()]
            if not names:
                raise ValueError(f"{class_map_file} is empty; provide one class name per line.")
            original = {v: k for k, v in self.class_to_idx.items()}
            if set(names) != set(original.values()):
                missing = sorted(set(original.values()) - set(names))
                extra = sorted(set(names) - set(original.values()))
                problems = []
                if missing:
                    problems.append(f"missing classes {missing}")
                if extra:
                    problems.append(f"unknown classes {extra}")
                raise ValueError(
                    f"Class names in {class_map_file} must match dataset folders; {', '.join(problems)}."
                )
            name_to_new = {name: i for i, name in enumerate(names)}
            idx_remap = {old_idx: name_to_new[original[old_idx]] for old_idx in original}
            self.classes = names
            self.class_to_idx = name_to_new.copy()
            self.samples = [(path, idx_remap[target]) for path, target in self.samples]
